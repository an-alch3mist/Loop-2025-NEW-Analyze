...
# Number * String  
10 * "="  
# Result is a proper C# string
("name " * 2).trim().join("").replace("a", "b").split(" ")

(["a", "qwerty"].join(",") * 3).strip().replace("a", " ").lower().upper().split() 
(" name " * 3).strip().replace("---", " ").lower().upper().split() 
```
...
# 0. could you include type(any object) # be it a int float many more, i guess just call them number since numberHandling does it all
so any val `<class "number">` what you think ? or `<class 'int'>`  `<class 'float'>` feel free to do what you prefer same goes for list, dict, class, none,bool many more similar to traditional python, so i could do contitional match too such as type(somthng) == what i need, 
btw tryna build a `farmer was replaced` clone for educational purpose
here's the error log report [pasted] from unity console(log reports) what you think is the issue ? provide the fix and let me know.
what's the file you gotta need to modify to fix that, and provide the required solution.
make sure it wont break any the existing features that is successfully working
...
# 1. also quick question imagine there is a game builtin function which returns stringwith multi line) time budget independent
```py
def _A():
    lines = get_txt().split('\n') # will the split operation chainable and many more work on gameInbuilt too or what?
_A()
```

# 2. when i run follwing
```py
def _A():
    list = [0, 1, 2].append(100)
    print(list)
    return "done"
print(_A())
```
unity output console(there is none instead of what it should be [0, 1, 2, 100] no ? make sure chainable on list works too such as .sort().append().remove()) many more or remove with index just like traditional python.
```
None
done
<color=#88cc00>[Execution complete]</color>
